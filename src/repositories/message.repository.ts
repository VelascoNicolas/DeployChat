...
export class MessageRepository extends GenericRepository<Message> {
  ...
  //GetAll de mensaje hasta con un maximo de 3 hijos
  public async getMessagesWithMessages(idEnterprise: string): Promise<Message[]> {
    try {
      const entities = await this.repository.find({
        where: {
          isDeleted: false,
        ...
        relations: [
          ...
          "childMessage.childMessage.childMessage",
        ],
        order: {
          numOrder: "ASC",
        },
  ...
  // GetOne demensaje específico con un máximo de 3 hijos
  public async getOneWithMessages(id: string, idEnterprise: string): Promise<Message> {
    try {
      const entity = await this.repository.findOne({
        where: {
          id: id,
  ...
  // GetAll de option == "MENU" para los padres y trae hasta 3 hijos sin importar su option
  public async getMessagesWithMenuMessages(idEnterprise: string): Promise<Message[]> {
    try {
      const entities = await this.repository.find({
        where: {
          isDeleted: false,
  ...
  // GetOne de mensaje específico hasta un máximo de 3 hijos, solo el mensaje padre con option == "MENU"
  public async getOneWithMenuMessages(id: string, idEnterprise: string): Promise<Message> {
    try {
      const entity = await this.repository.findOne({
        where: {
          id: id,
  ...
  public async updateMessage(
      ...
      try {
        ...
        message.typeMessage = data.typeMessage?? message.typeMessage;
