...
export class FlowRepository extends GenericRepository<Flow> {
  ...
  // Obtener un flujo específico con sus mensajes y hijos
  public async getOneWithMessagesAndMessages(id: string, idEnterprise: string): Promise<Flow | undefined> {
    try {
      ...
      const messageRepository = new MessageRepository();
      entity.messages = await messageRepository.getMessagesWithMessages(idEnterprise);
  ...
  // Obtener todos los flujos con sus mensajes y hijos
  public async getAllWithMessagesAndMessages(idEnterprise: string): Promise<Flow[]> {
    try {
      const enterpriseRepository = new EnterpriseRepository();
      const entityEnterprise = await enterpriseRepository.findOne({
        where: { id: idEnterprise },
      ...
      const messageRepository = new MessageRepository();
      const allMessagesWithSubMessages = await messageRepository.getMessagesWithMessages(idEnterprise);
  ...
  // Obtener un flujo específico con sus mensajes con option == "MENU" y hasta 3 hijos
  public async getOneWithMenuMessagesAndMessages(id: string, idEnterprise: string): Promise<Flow | undefined> {
    try {
      const entity = await this.repository.findOne({
        where: { id, isDeleted: false },
      });
      ...
      const messageRepository = new MessageRepository();
      entity.messages = await messageRepository.getMessagesWithMenuMessages(idEnterprise);
  ...
  // Obtener todos los flujos con sus mensajes con option == "MENU" y hasta 3 hijos
  public async getAllWithMenuMessagesAndMessages(idEnterprise: string): Promise<Flow[]> {
    try {
      const enterpriseRepository = new EnterpriseRepository();
      const entityEnterprise = await enterpriseRepository.findOne({
        where: { id: idEnterprise },
      ...
      const messageRepository = new MessageRepository();
      const allMessagesWithMenuSubMessages = await messageRepository.getMessagesWithMenuMessages(idEnterprise);

      // Asignar mensajes con option "MENU" a cada flujo y eliminar `flow` de cada mensaje
      for (const flow of flows) {
        flow.messages = allMessagesWithMenuSubMessages
          .filter((message) => message.flow && message.flow.id === flow.id)
          .map((message) => {
            const { flow, ...messageWithoutFlow } = message;
            return messageWithoutFlow;
