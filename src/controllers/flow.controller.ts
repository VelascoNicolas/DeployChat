...
export class FlowController extends GenericController<Flow, FlowDto> {
  ...
  async findAllFlowsWithMessages(req: Request, res: Response) {
    try {
      ...
      const flows = await this.flowRepository.getAllWithMessagesAndMessages(idEnterprise);
  ...
  // GetOne obtiene un flujo específico con sus mensajes y hijos
  async getOneWithMessagesAndMessages(req: Request, res: Response) {
    try {
      const id = req.params.id;
      const idEnterprise = await this.getEnterpriseId(req, res);
      const flow = await this.flowRepository.getOneWithMessagesAndMessages(id, idEnterprise);
  ...
  async getAllWithMessagesAndMessages(req: Request, res: Response) {
      try {
          const idEnterprise = await this.getEnterpriseId(req, res);
          const flows = await this.flowRepository.getAllWithMessagesAndMessages(idEnterprise);
          const flowsDto = flows.map((flow) => toDtoFromEntity(FlowDto, flow));
  ...
  async getOneWithMenuMessagesAndMessages(req: Request, res: Response) {
    try {
      const id = req.params.id;
      const idEnterprise = await this.getEnterpriseId(req, res);
      const flow = await this.flowRepository.getOneWithMenuMessagesAndMessages(id, idEnterprise);

      if (!flow) {
        return res.status(404).json({ message: "Flow not found, getOneWithMenuMessagesAndMessages" });
      }
  ...
  async getAllWithMenuMessagesAndMessages(req: Request, res: Response) {
      try {
          const idEnterprise = await this.getEnterpriseId(req, res);
          const flows = await this.flowRepository.getAllWithMenuMessagesAndMessages(idEnterprise);
          const flowsDto = flows.map((flow) => toDtoFromEntity(FlowDto, flow));
          return res.status(200).json(flowsDto);
      } catch (error) {
          return handleErrors(error, res);
