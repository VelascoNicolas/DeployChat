...
export class MessageController extends GenericController<Message, MessageDto> {
  ...
  async getMessagesWithMessages(req: Request, res: Response) {
    try {
      const idEnterprise = await this.getEnterpriseId(req, res);
      const entities = await this.messageRepository.getMessagesWithMessages(idEnterprise);
  ...
  async getOneWithMessages(req: Request, res: Response) {
    try {
      const id = req.params.id;
      const idEnterprise = await this.getEnterpriseId(req, res);
      const entity = await this.messageRepository.getOneWithMessages(id, idEnterprise);
  ...
  // Obtener todos los mensajes con sus 3 hijos por option == "MENU"
  async getMessagesWithMenuMessages(req: Request, res: Response) {
    try {
      const idEnterprise = await this.getEnterpriseId(req, res);
      const entities = await this.messageRepository.getMessagesWithMenuMessages(idEnterprise);
      const messagesDto = entities.map((message) => toDtoFromEntity(MessageDto, message));
  ...
  // Obtener un mensaje por ID con sus 3 hijos por option == "MENU"
  async getOneWithMenuMessages(req: Request, res: Response) {
    try {
      const id = req.params.id;
      const idEnterprise = await this.getEnterpriseId(req, res);
      const entity = await this.messageRepository.getOneWithMenuMessages(id, idEnterprise);
